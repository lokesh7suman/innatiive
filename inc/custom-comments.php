...
add_filter( 'comment_form_default_fields', 'innatiive_bootstrap_comment_form_fields' );

if ( ! function_exists( 'innatiive_bootstrap_comment_form_fields' ) ) {
	/**
	 ...
	 * @return array
	 */
	function innatiive_bootstrap_comment_form_fields( $fields ) {
...
} // End of if function_exists( 'innatiive_bootstrap_comment_form_fields' )

// Add Bootstrap classes to comment form submit button and comment field.
add_filter( 'comment_form_defaults', 'innatiive_bootstrap_comment_form' );

if ( ! function_exists( 'innatiive_bootstrap_comment_form' ) ) {
	/**
	 ...
	 * @return string[]
	 */
	function innatiive_bootstrap_comment_form( $args ) {
		$replace = array(
			'<p class="' => '<div class="form-group ',
...
} // End of if function_exists( 'innatiive_bootstrap_comment_form' ).


// Add note if comments are closed.
add_action( 'comment_form_comments_closed', 'innatiive_comment_form_comments_closed' );

if ( ! function_exists( 'innatiive_comment_form_comments_closed' ) ) {
	/**
	 * Displays a note that comments are closed if comments are closed and there are comments.
	 */
	function innatiive_comment_form_comments_closed() {
		if ( get_comments_number() && post_type_supports( get_post_type(), 'comments' ) ) {
			?>
			<p class="no-comments"><?php esc_html_e( 'Comments are closed.', 'innatiive' ); ?></p>
			<?php
		}
	}
} // End of if function_exists( 'innatiive_comment_form_comments_closed' ).
